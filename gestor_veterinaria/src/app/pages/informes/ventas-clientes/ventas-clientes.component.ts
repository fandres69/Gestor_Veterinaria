...
import { AniosV, dataSetDetail, VentasModel, FiltroProduct, ChartOptions, FiltroCliente } from '../interfaces/ventas';
...
export class VentasClientesComponent implements OnInit {
  ...
  InfoGraph(){
    const productos:FiltroCliente[]=[];
    this.ventasVList.forEach(element => {
      let sum=0;
      const filtro= productos.filter(x=>x.cliente===element.cliente);
      if(filtro.length===0){
        let listado=this.ventasVList.filter(x=>x.cliente===element.cliente);
        if(listado.length>0){
          listado.forEach(val => {
            sum+=val.Total||0;
          });
          productos.push({cliente:element.cliente,nombreCliente:element.nombreCliente,total:sum});
          this.labelsTag.push(`${element.cliente}-${element.nombreCliente}`||'');
          this.data.push(sum);
  ...
  infoProductosMes(){
    this.lineSerieT=[];
    let datFiltro:VentasModel[]=[];
    const productos_:FiltroCliente[]=[];
    ...
    if(this.yearS===0){
       ...
       prod.forEach(element => {
        let nomPro=element.nombreCliente||'';
        let clienDoc=element.cliente||0;
        let existe=productos_.filter(x=>x.nombreCliente===nomPro && x.cliente===clienDoc);
        if(existe.length===0){
          productos_.push({nombreCliente:nomPro,cliente:clienDoc});
        }
      });
      let yYears=this.yearList; 
      productos_.forEach(product => {
        const dataTotal:number[]=[];
          ...
          let sum=0;
          datFiltro = this.VentasL.filter(x=> x.anio===anio_ && x.cliente=== product.cliente);
        ...
        let name=(product.cliente+'-'+product.nombreCliente);
      ...
      productos_.forEach(product => {
        ...
        for (let i = 0; i < 12; i++) {
          ...
          const dt=datFiltro.filter(x=>x.mes===i && x.cliente===product.cliente);
         if(dt.length>0){
           dt.forEach(element => {
             sum+=element.Total||0;  
           }); 
