...
@Component({
  selector: 'app-ventas-servicios',
  templateUrl: './ventas-servicios.component.html',
  styleUrls: ['./ventas-servicios.component.css']
})
export class VentasServiciosComponent implements OnInit {
  ...
  public cOPtios:ChartOptions={
    ...
    title:{
      text:`Ventas por Servicio año ${(this.yearS>0)?this.yearS:'Todos'}`
    },
    xaxis:{
      categories:this.labelsTag||['servicios']
    }
  };
  ...
  public titlePie:ApexTitleSubtitle={text:`% servicios en ventas por año ${(this.yearS>0)?this.yearS:'Todos'}`,align:'center'};
  ...
  InfoGraph(){
    ...
    this.ventasVList.forEach(element => {
      ...
      if(filtro.length===0){
        let listado=this.ventasVList.filter(x=>x.producto===element.producto && x.tipoProducto==='S');
        if(listado.length>0){
          listado.forEach(val => {
            sum+=val.Total||0;
          });
  ...
  infoGraphLine(){
    ...
    if(this.yearS===0){
        ...
        let sum=0;
        datFiltro = this.VentasL.filter(x=> x.anio===anio_ && x.tipoProducto==='S');
        for (let i = 0; i < 12; i++) {
          sum=0;
      ...
      const dataTotal:number[]=[];
      datFiltro = this.VentasL.filter(x=>x.tipoProducto==='S' && x.anio===this.yearS);
      for (let i = 0; i < 12; i++) {
         let sum=0;
  ...
  public titleGLT:ApexTitleSubtitle={text:`Ventas totales por servicios año: ${(this.yearS>0)?this.yearS:'Todos'}`,align:'center'};
  ...
  infoProductosMes(){
    ...
    if(this.yearS===0){
       let prod=this.ventasVList.filter(x=>x.tipoProducto==='S');
      ...
      productos_.forEach(product => {
        const dataTotal:number[]=[];
          ...
          let sum=0;
          datFiltro = this.VentasL.filter(x=> x.anio===anio_ && x.tipoProducto==='S' && x.producto=== product);
