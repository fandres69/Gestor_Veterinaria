...
@Component({
  selector: 'app-upd-detalle',
  templateUrl: './upd-detalle.component.html',
  styleUrls:['./detalles.css']
})
export class UpdDetalleComponent implements OnInit {
  ...
  private vistaDetalleUpdate!:VistaDetalle;
...
editarDetallesOrd(){
  ...
  const cantidad =parseInt(this.miForm.get('cantidad')?.value||0);   
  const inv=this.detalleInvL.filter(x=>x.idProductos==this.vistaDetalleUpdate.producto);
  let saldo=(inv[0].stock||0)+(this.vistaDetalleUpdate.cantidad||0 )-cantidad;
  if(saldo<0){
    Swal.fire({
      icon: 'error',
      title: 'Oops...',
...
updateDetalle(cantidad:number){
    const det:DetallePedModel={
      iddetallePedido:this.vistaDetalleUpdate.iddetallePedido,
      producto:this.vistaDetalleUpdate.producto,
      cantidad:cantidad,
      precio:this.vistaDetalleUpdate.precio,
      impuesto:this.vistaDetalleUpdate.impuesto,
      cliente:this.vistaDetalleUpdate.clienteDoc,
      ciudad:this.vistaDetalleUpdate.ciudad,
      pedido:this.vistaDetalleUpdate.pedido,
      tipoProducto:this.vistaDetalleUpdate.tipoProducto,
      dia:this.vistaDetalleUpdate.dia,
      mes:this.vistaDetalleUpdate.mes,
      anio:this.vistaDetalleUpdate.anio,
      descuento:this.vistaDetalleUpdate.descuento
    }

  this.salesMng.updateDetalle(det).subscribe(resp=>{
    if(resp.error){
